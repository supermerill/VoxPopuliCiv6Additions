...
--Controls["EurekaUpdateButton"]:RegisterCallback(Mouse.eLClick, GenericLeftClick);
--Controls["EurekaUpdateButton"]:RegisterCallback(Mouse.eRClick, GenericRightClick);
...
--eureka
g_NameTable[ NotificationTypes.NOTIFICATION_EUREKA_UPDATE ] = "EurekaUpdate";
...
function OnNotificationAdded( Id, type, toolTip, strSummary, iGameValue, iExtraGameData, ePlayer )
    ...
    if( name == "Production" or
		...
		elseif type == NotificationTypes.NOTIFICATION_EUREKA_UPDATE then
			local thisTechInfo = GameInfo.Technologies[iExtraGameData];
			local portraitIndex = thisTechInfo.PortraitIndex;
			if portraitIndex ~= -1 then
				IconHookup( portraitIndex, 80, thisTechInfo.IconAtlas, instance.EurekaUpdateImage );				
			else
				instance.EurekaUpdateImage:SetHide( true );
			end
