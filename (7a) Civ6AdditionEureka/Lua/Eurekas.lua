...
if(iTeam >= 0 and iPlotX >= 0 and iPlotY >=0) then
	...
	if(iTerrainType == TERRAIN_TYPE_SEA) then
...
print("Eureka: GreatPersonExpended!".. iPlayer);
if(iPlayer >=0 and iUnit >=0)then 
	...
	local pTeam = Teams[pPlayer:GetTeam()];
	local pTeamTech = pTeam:GetTeamTechs();
print("Eureka: GreatPersonExpended! type = ".. iUnitType);
...
if(iPlayer >=0 and iUnit>=0) then
	...
	local pUnit = pPlayer:GetUnitByID(iUnit);
	local iUnitType = pUnit:GetUnitType();
	print(string.format("Eureka: CityTrained unit %s",pUnit:GetName()));
