...
function ActionToolTipHandler( control )
	...
	if isBuild then
		...
		local buildStrengthNeeded = g_activePlayer:GetImprovementBuilderCost(buildID)
		if buildStrengthNeeded > 0 then
			strBuildTurnsString = "   " .. L("TXT_KEY_STR_BUILDER_COST", buildStrengthNeeded )
		end
		...
		if feature and plot:IsBuildRemovesFeature(buildID) then
			...
			if improvement then
				...
				strBuildTurnsString = strBuildTurnsString .. " + " .. L("TXT_KEY_STR_BUILDER_COST_SMALL", 100 )
			end
